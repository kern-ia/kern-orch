...
* **Update**: Issue 13 (#24, opt-in runtime replay-equivalence check) implemented on `feature/runtime-equivalence-check`. Wires issue 10's own comparison (full-state JSON encoding) into a `Config.RuntimeEquivalenceCheck` field, off by default; enabled it names the divergent keys, disabled it adds zero projection calls (proven by call-count instrumentation, not timing). `FromEnv()` now returns `(Config, error)` so an invalid value fails loud at load. This is the epic's last issue.
