...
### How the pieces fit
`cmd` reads `config` (env) → builds an `AgentRunner` via `agentrunner.New` (the adapter named by `KERN_AGENT_KIND` if `KERN_AGENT_CLI` is set, else `agentrunner.Stub`) → `topology.Load` turns a YAML graph into a `graph.Graph`, resolving `tool`/`router` names against a `topology.Registry` of Go funcs and backing `agent` nodes with the runner → `graph.Engine` runs it level-synchronously, reporting what it does through an `OnEvent` hook and calling an `OnStep` hook that persists each level to the `checkpoint` store. **The journal is the source of truth**: the `checkpoints` row is a materialized projection of a run's events, written in the same transaction as the events of its level (`checkpoint.AppendAndProject`), never independently — its state comes from `journal/projection.Project`, not from marshalling the live `*graph.State`. `resume` rebuilds the state by **replaying the run's journal** (`checkpoint.ResumePoint`) and calls `Engine.RunFrom` — never from the cached row's state, which is a cache and may be stale; a run that has a checkpoint but no events at all is refused (`checkpoint.ErrNoJournal`) rather than resumed from an empty state. A journal that carries no terminal event at all is a run nobody closed (a hard kill mid-level, or a stop whose terminal events were refused on the run's own cancelled context): resume closes it first, appending a `NodeFailed` per node left unresolved and a `RunInterrupted`, each flagged `Synthetic` on the envelope so a reader can tell a reconstructed event from an observed one. No synthetic `LevelClosed` is ever written — the open level is what resume restarts, and closing it would fold partial branches into a state the run never held. A journal that already ends on a terminal event is left byte-identical. The row still supplies the provenance the journal does not carry, so `resume <run-id>` needs no graph argument, and the CLI and `POST /api/v1/runs/{id}/resume` go through that same call. Dependency direction: `graph` defines the `AgentRunner` port and the `StepFunc` hook; `agentrunner` and `checkpoint` depend on `graph`, never the reverse.
...
The placeholder JSON-lines protocol is **gone** (epic 2, issue 04): spec §6.4 is now settled per CLI, by adapter. `agentrunner.ClaudeCode` speaks Claude Code's real `-p --input-format=stream-json --output-format=stream-json --verbose` protocol; `agentrunner.New` dispatches on `KERN_AGENT_KIND`. `agentrunner.OpenCode` (epic 2, issue 05) speaks the real `opencode serve` HTTP protocol — one session per `Run()` call, proven to stay genuinely concurrent under a fan-out rather than serialize behind a lock (epic 2, issue 06). The `topology.Registry` ships only a `noop` builtin tool; real projects register their own tool/router funcs in Go.
