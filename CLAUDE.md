...
The placeholder JSON-lines protocol is **gone** (epic 2, issue 04): spec §6.4 is now settled per CLI, by adapter. `agentrunner.ClaudeCode` speaks Claude Code's real `-p --input-format=stream-json --output-format=stream-json --verbose` protocol; `agentrunner.New` dispatches on `KERN_AGENT_KIND`. The `opencode` kind is still a placeholder returning an error (epic 2, issue 05). The `topology.Registry` ships only a `noop` builtin tool; real projects register their own tool/router funcs in Go.
